...
# teardown_file().  The container is placed on a private Docker network with
# the alias "amazeeio-mail" so that tests which explicitly set
# SSMTP_MAILHUB=amazeeio-mail:1025 (the first branch of 50-ssmtp.sh) can
# resolve that hostname.  Note: 50-ssmtp.sh's auto-detection probes use
# 172.17.0.1:1025 and host.docker.internal:1025 — not "amazeeio-mail".
# Host-mapped ports are used for curl-based SMTP sends and API assertions so
# that the tests work on both Linux and macOS (Docker Desktop).
#
...
setup_file() {
    ...
    # reach Mailpit via the "amazeeio-mail" hostname without any extra setup.
    ...
    docker network connect --alias amazeeio-mail "${isolated_network}" "${container}"
...
setup() {
    local suffix
    suffix="$(cat "${BATS_SUITE_TMPDIR}/.suffix" 2>/dev/null || true)"
    MAIL_CONTAINER="mailpit-bats-test-${suffix}"
    MAIL_NETWORK="$(cat "${BATS_SUITE_TMPDIR}/.network" 2>/dev/null || true)"
    MAIL_ISOLATED_NETWORK="$(cat "${BATS_SUITE_TMPDIR}/.isolated_network" 2>/dev/null || true)"
...
@test "container is running" {
    run docker inspect --format='{{.State.Status}}' "${MAIL_CONTAINER}"
    [ "$status" -eq 0 ]
    [ "$output" = "running" ]
}

@test "mailpit process is running inside the container" {
    run docker exec "${MAIL_CONTAINER}" sh -c 'ps | grep "[m]ailpit"'
    [ "$status" -eq 0 ]
    [ -n "$output" ]
}
...
@test "email sent via Lagoon PHP container (ssmtp) is captured by Mailpit" {
    ...
    # Pass SSMTP_MAILHUB so 50-ssmtp.sh writes mailhub=amazeeio-mail:1025
    # into /etc/ssmtp/ssmtp.conf, then send via the real ssmtp sendmail binary.
    run docker run --rm \
        --network "${MAIL_NETWORK}" \
        -e "SSMTP_MAILHUB=amazeeio-mail:1025" \
        --entrypoint sh \
        "${PHP_FPM_IMAGE}" -euc '
...
@test "email sent with SSMTP_MAILHUB set to 'amazeeio-mail:1025' is captured by Mailpit" {
    delete_all_messages
...
@test "email sent via host.docker.internal route is captured by Mailpit" {
    ...
    local mail_isolated_ip
    mail_isolated_ip="$(docker inspect \
        --format="{{(index .NetworkSettings.Networks \"${MAIL_ISOLATED_NETWORK}\").IPAddress}}" \
        "${MAIL_CONTAINER}")"

    run docker run --rm \
        --network "${MAIL_ISOLATED_NETWORK}" \
        --add-host "host.docker.internal:${mail_isolated_ip}" \
        --entrypoint sh \
        "${PHP_FPM_IMAGE}" -euc '
            . /lagoon/entrypoints/50-ssmtp.sh
